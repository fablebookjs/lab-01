...
The manual operator-only exact \`1.0.0\` bootstrap completed on 2026-07-16; both public tarballs match immutable tag \`v1.0.0\`, and a clean external consumer installed them together. The trusted-main maintainer, Ready-QA controller, \`V\` preparation, direct-OIDC publisher, finalizer, and H/J handoff are installed but have not completed a real patch release. The \`npm-publish\` environment exists with no secrets or reviewers and permits only \`main\`. Both npm package pages trust only \`fablebookjs/lab-01\` workflow \`publish-npm.yml\`, environment \`npm-publish\`, and action \`npm publish\`; both require 2FA and disallow traditional tokens. Current-head QA is successful.

This maintainer never publishes, reconciles the release line, tags, or creates a GitHub Release. It validates the open \`1.0.1\` proposal, sealed merge \`M\`, exact deterministic \`V\`, concrete \`H\`, deterministic \`J\`, and an exact draft \`1.0.2\` proposal from trusted-main code. Caller-supplied H/J facts are rejected; two complete ownership snapshots rederive and reclassify the durable graph. The next operator action is the exact current-ready merge; package publication remains unavailable until trusted-main preparation records \`M\` and \`V\`.
