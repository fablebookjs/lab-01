...
`inject-after-force` verifies the remote backup at `H`, moves only the calibration
line from `H` to `V` with an expected-`H` force-with-lease, records the identities,
and intentionally fails before any PR operation. `resume` accepts only that exact
backup and a line at `H` or `V`, performs the guarded force only when needed, and
creates or reuses one same-repository draft recovery PR from `backup` to `line`.
